...
def visualize_explanation(
    ...
    # cbar_ax.xaxis.set_ticks([0, max_val/2, max_val])
    # cbar_ax.xaxis.set_ticklabels(["Low", "Medium", "High"], fontsize=9)
...
def generate_explanations(args, model_type):
    ...
    # 为每个类别创建独立文件夹
    for class_id in range(0, 19):  # 类别1到22
    ...
    for exp_vis in args.ques:
        ...
        while not all_classes_complete:
            all_classes_complete = True  # 假设所有类别都已完成收集
            ...
            for i, (X, y, orig_sizes, _) in enumerate(test_loader):
                ...
                # 打印当前样本信息
                print(f"样本 {i}: 真实标签 {y_true} ({MTARSI_CLASS_NAMES[y_true]}), " 
                      f"预测 {y_pred} ({MTARSI_CLASS_NAMES.get(y_pred, 'unknown')}) - "
